...
def test__replace_size_info_with_selected_size_info__updates_text_with_one_set_of_sizing_info():
    ...
    output = SizeInfoUpdater.replace_size_info_with_selected_size_info(text, set_of_size_info, size)
...
def test__replace_size_info_with_selected_size_info__updates_text_with_multiple_sets_of_sizing_info(size, expected):
    ...
    output = SizeInfoUpdater.replace_size_info_with_selected_size_info(text,
...
def test__replace_size_info_with_selected_size_info__removes_parentheses_from_size_info():
    parentheses = ["(", ")"]
    SizeInfoUpdater.trim_size_info = Mock()
    SizeInfoUpdater.trim_size_info.return_value = "5"

    SizeInfoUpdater.replace_size_info_with_selected_size_info("2 (3) 4 (5) 6 balls of yarn", ["2 (3) 4 (5) 6"], 3)

    SizeInfoUpdater.trim_size_info.assert_called_with(parentheses, "(5)")
...
def test__replace_size_info_with_selected_size_info__returns_trimmed_size_info():
    ...
    output = SizeInfoUpdater.replace_size_info_with_selected_size_info("2 (3) 4 (5) 6 balls of yarn", ["2 (3) 4 (5) 6"],
                                                                       3)
    assert str(output) == expected
